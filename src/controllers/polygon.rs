...
impl PolygonController {
    ...
    async fn ensure_configmaps(
        ...
        configmaps
            .patch(
                ...
                &PatchParams::apply("cto-operator").force(),
    ...
    fn generate_heimdall_config(&self, spec: &PolygonNodeSpec) -> String {
        ...
        let chain_id = match spec.network {
            PolygonNetwork::Mainnet => "heimdallv2-137",
...
"#,
    ...
    fn eth_rpc_url(&self, spec: &PolygonNodeSpec) -> String {
        match spec.network {
            PolygonNetwork::Mainnet => "https://eth-mainnet.public.blastapi.io".to_string(),
            PolygonNetwork::Amoy => "https://eth-sepolia.public.blastapi.io".to_string(),
        }
    }

    fn bor_rpc_url(&self, spec: &PolygonNodeSpec) -> String {
        match spec.node_type {
...
"#,
    ...
    async fn ensure_statefulset(
        ...
        // Init container 1: Initialize Heimdall (creates priv_validator_key.json, node_key.json, etc.)
        let mut init_containers = vec![Container {
            name: "heimdall-init".to_string(),
            image: Some(spec.heimdall.image.clone()),
            command: Some(vec!["sh".to_string(), "-c".to_string()]),
            args: Some(vec![format!(
                "if [ ! -f /heimdall-data/config/priv_validator_key.json ]; then heimdalld init cto-polygon-node --home=/heimdall-data --chain-id={} --chain={} 2>&1 || true; fi",
                chain_id,
                match spec.network {
                    PolygonNetwork::Mainnet => "mainnet",
                    PolygonNetwork::Amoy => "amoy",
        ...
        // Init container 2: Download genesis (mainnet migrated genesis is ~4-5GB)
        // The genesis URL provides the real network genesis which replaces the default
        // generated by `heimdalld init`. We check file size to detect the placeholder.
        if let Some(ref genesis_url) = spec.heimdall.genesis_url {
            init_containers.push(Container {
                ...
                command: Some(vec!["sh".to_string(), "-c".to_string()]),
                args: Some(vec![format!(
                    "GENESIS=/heimdall-data/config/genesis.json; SIZE=$(wc -c < \"$GENESIS\" 2>/dev/null || echo 0); echo \"Current genesis size: $SIZE bytes\"; if [ \"$SIZE\" -lt 1000000 ]; then echo 'Downloading genesis from {}...' && wget -q -O \"$GENESIS\" '{}' && NEWSIZE=$(wc -c < \"$GENESIS\"); echo \"Genesis downloaded: $NEWSIZE bytes\"; else echo 'Genesis already present, skipping download'; fi",
        ...
        // ConfigMap volumes (Bor config only — Heimdall uses init-generated config + CLI flags)
        ...
        statefulsets
            .patch(name, &PatchParams::apply("cto-operator").force(), &Patch::Apply(sts))
            .await?;
    ...
    fn heimdall_container(&self, spec: &PolygonNodeSpec) -> Container {
        let mut args = vec![
            ...
            format!("--eth_rpc_url={}", self.eth_rpc_url(spec)),
            format!("--bor_rpc_url={}", self.bor_rpc_url(spec)),
            format!("--heimdall_rest_server=http://0.0.0.0:{}", spec.heimdall.rest_port),
        ];

        // Pass seeds via CLI flag to override init-generated config
        if let Some(ref seeds) = spec.heimdall.seeds {
            args.push(format!("--p2p.seeds={}", seeds.join(",")));
        }
        ...
        Container {
            ...
            // Startup probe: allows up to 30 min for genesis processing
            // (failureThreshold=180 * periodSeconds=10 = 1800s)
            // Uses CometBFT RPC /status endpoint on port 26657
            startup_probe: Some(Probe {
                http_get: Some(HTTPGetAction {
                    path: Some("/status".to_string()),
                    port: IntOrString::Int(spec.heimdall.rpc_port),
                    ..Default::default()
                }),
    ...
    fn bor_container(&self, spec: &PolygonNodeSpec, bor: &BorConfig) -> Container {
        ...
        if spec.enable_metrics {
            ...
            args.push("--pprof.addr=0.0.0.0".to_string());
            args.push(format!("--pprof.port={}", bor.metrics_port));
        }
        ...
        Container {
            ...
            // Wait for Heimdall REST API before starting (up to 35 min)
            // Heimdall genesis processing can take 10-30 min on mainnet
            startup_probe: Some(Probe {
                exec: Some(ExecAction {
                    command: Some(vec![
                        "sh".to_string(),
                        "-c".to_string(),
                        format!("wget -q -O- http://localhost:{}/status > /dev/null 2>&1", spec.heimdall.rpc_port),
                    ]),
                }),
                failure_threshold: Some(210),
                period_seconds: Some(10),
                ..Default::default()
            }),
            readiness_probe: Some(Probe {
    ...
    fn erigon_container(&self, spec: &PolygonNodeSpec, erigon: &ErigonConfig) -> Container {
        ...
        if spec.enable_metrics {
            args.push("--metrics".to_string());
            args.push("--metrics.addr=0.0.0.0".to_string());
            args.push(format!("--metrics.port={}", erigon.metrics_port));
        }
